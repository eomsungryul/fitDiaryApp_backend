...
public class ConsultingInfoController {
    ...
    @PostMapping
    public Result add(@RequestBody ConsultingInfo consultingInfo) {
        consultingInfoService.save(consultingInfo);
        return ResultGenerator.genSuccessResult();
    }

    @DeleteMapping("/{id}")
    public Result delete(@PathVariable Integer id) {
        consultingInfoService.deleteById(id);
        return ResultGenerator.genSuccessResult();
    }
    ...
    public Result detail(@PathVariable Integer id) {
        ...
        return ResultGenerator.genSuccessResult(consultingInfo);
    }

    @GetMapping
    public Result list(@RequestParam(defaultValue = "0") Integer page, @RequestParam(defaultValue = "0") Integer size) {
        PageHelper.startPage(page, size);
        List<ConsultingInfo> list = consultingInfoService.findAll();
